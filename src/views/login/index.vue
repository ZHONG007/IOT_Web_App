    ...
    return {
      ...
      formRules: { // 表单验证规则配置
        ...
        email: [
          ...
          { pattern: /^1[3|5|7|8|9]\d{9}$/, message: 'Invalid Email Address', trigger: 'blur' }
        ],
        password: [
          { required: true, message: 'Empty', trigger: 'blur' },
          { pattern: /^\d{6}$/, message: 'Invalid Password', trigger: 'blur' }
        ],
        agree: [
          {
            ...
            validator: (rule, value, callback) => {
              if (value) {
                ...
                // 验证失败
                callback(new Error('You need accept term of use before proceed.'))
              }
            },
            // message: '请勾选同意用户协议',
            trigger: 'blur'
